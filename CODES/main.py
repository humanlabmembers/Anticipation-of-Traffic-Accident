...
def test_all(testdata_loader, model):
    ...
    with torch.no_grad():
        for i, (features, labels, toa,detection,depth) in enumerate(testdata_loader):
            losses, all_outputs= model(features, labels, toa,detection,depth)
...
def test_all_vis(testdata_loader, model, vis=False, multiGPU=False, device=torch.device('cuda')):
    ...
    with torch.no_grad():
        for i, (features, labels, toa,detection,depth) in tqdm(enumerate(testdata_loader), desc="batch progress", total=len(testdata_loader)):
...
def train_eval():
    ...
    # create data loader
    if p.dataset == 'dad':
        train_data = DADDataset(data_path, p.feature_name, phase='training', toTensor=True, device=device)
        test_data = DADDataset(data_path, p.feature_name, phase='testing', toTensor=True, device=device)
    else:
    ...
    for k in range(p.epoch):
        ...
        if k <= start_epoch:
            ...
            continue
        for i, (features, labels, toa,detection,depth) in loop:
            ...
            losses['total_loss'] = losses['total_loss'] + l1_loss
...
def test_eval():
    ...
    if p.evaluate_all:
        ...
        # run model inference
        all_pred, all_labels, all_toas, vis_data = test_all_vis(testdata_loader, model, vis=False, device=device)
        # save predictions
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='dad', choices=['dad'],
    ...
    parser.add_argument('--test_iter', type=int, default=127,
    ...
    parser.add_argument('--output_dir', type=str, default=your_out_dir,
